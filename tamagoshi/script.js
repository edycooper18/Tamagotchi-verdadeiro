document.addEventListener('DOMContentLoaded', () => {
    ...
    const PET_STAGES = [
        { name: 'Devorador de Casas', emoji: '🥚', minAge: 0, maxAge: 1 },
        { name: 'Devorador de Cidades', emoji: '🐣', minAge: 1, maxAge: 5 },
        { name: 'Devorador de Paises', emoji: '🐥', minAge: 5, maxAge: 15 },
        { name: 'Devorador de Mundos', emoji: '🐤', minAge: 15, maxAge: 30 },
        { name: 'Devorador de Galaxias', emoji: '🐔', minAge: 30, maxAge: Infinity }
    ];

    // --- Estado do Tamagotchi ---
    let tamagotchi = {
        name: 'Devorador de Casas',
        hunger: MAX_STAT,
        happiness: MAX_STAT,
        energy: MAX_STAT,
    ...
    function getPetEmoji() {
        if (!tamagotchi.isAlive) return '☠️'; // Fantasma se estiver morto
    ...
    function performAction(actionType) {
        if (!tamagotchi.isAlive) {
            showMessage('Esse ser cosmicamente épico foi de expoxed do Felca.');
            return;
        }
        if (tamagotchi.isSleeping) {
            showMessage('Apos muitos atos cosmicos esse ser se encontra em estado de sonolencia');
            return;
        }
        ...
        switch (actionType) {
            case 'feed':
                if (tamagotchi.hunger === MAX_STAT) {
                    showMessage('Esse ser cosmico está com seu estomago cosmico cheio.');
                    return;
                }
                tamagotchi.hunger = clampStat(tamagotchi.hunger + 20);
                tamagotchi.happiness = clampStat(tamagotchi.happiness + 5);
                showMessage('Você alimentou esse titãn cosmico');
                break;
            case 'play':
                if (tamagotchi.happiness === MAX_STAT) {
                    showMessage('O projenitor de poderes está feliz');
                    return;
                }
    ...
    function gameLoop() {
        ...
        if (ageCounter >= AGE_INTERVAL_SECONDS) {
            ...
            if (currentStage && tamagotchi.name !== currentStage.name) {
                ...
                showMessage(`Este ser evoluiu para a fase ${currentStage.name}!`);
            }
        }
